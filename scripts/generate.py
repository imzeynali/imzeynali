...
def demo_data() -> tuple[dict, dict]:
    ...
    profile = {
        ...
        "following": 18,
        "stars": 119,
        "languages": [["Python", 40], ["Linux", 30], ["React", 20], ["JavaScript", 10]],
    }
...
def identity_svg(cfg: dict) -> str:
    ...
    skills = "".join(f'<g transform="translate({337 + index * 87} 329)"><rect width="78" height="20" rx="10" fill="#0d2452" stroke="#1c47a0"/><text x="39" y="14" text-anchor="middle" class="chip">{escape(skill)}</text></g>' for index, skill in enumerate(cfg.get("skills", [])[:6]))
...
<defs><linearGradient id="shell" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#123a8f"/><stop offset=".56" stop-color="#0c1f4a"/><stop offset="1" stop-color="#2657c9"/></linearGradient><radialGradient id="aura"><stop stop-color="{TEAL}" stop-opacity=".22"/><stop offset="1" stop-color="{TEAL}" stop-opacity="0"/></radialGradient>{''.join(portrait_defs)}<clipPath id="type"><rect x="0" y="0" width="0" height="225"><animate attributeName="width" from="0" to="520" dur="2.5s" calcMode="spline" keySplines=".32 .72 0 1" fill="freeze"/></rect></clipPath><style>{css}</style></defs>
<rect width="900" height="380" rx="24" fill="{OUTER}"/><rect x="6" y="6" width="888" height="368" rx="20" fill="url(#shell)" stroke="{HAIR}"/><rect x="18" y="18" width="864" height="344" rx="17" fill="{BG}" stroke="#1e4fad"/><circle cx="230" cy="185" r="180" fill="url(#aura)"/>
...
<rect x="32" y="57" width="286" height="292" rx="22" fill="#0a1730" stroke="#1c47a0"/><text x="48" y="78" class="label">PORTRAIT.ASCII / @{escape(cfg['username'].upper())}</text><g class="portrait-ascii">{''.join(portrait_body)}</g>
<rect x="329" y="57" width="539" height="222" rx="22" fill="#0a1730" stroke="#1c47a0"/><g transform="translate(343 67)" clip-path="url(#type)" class="ascii"><g class="phase-a">{line_a}</g><g class="phase-b">{line_b}</g></g>
...
def contributions_svg(cfg: dict, contribution: dict) -> str:
    ...
    colors = ["#0d1b3d", "#153875", "#1c56ad", "#2f76e0", TEAL]
    nodes = []
...
<defs><linearGradient id="shell" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#12408f"/><stop offset=".58" stop-color="#0c1f4a"/><stop offset="1" stop-color="#2e5fbd"/></linearGradient><style>{css}</style></defs>
<rect width="900" height="292" rx="24" fill="{OUTER}"/><rect x="6" y="6" width="888" height="280" rx="20" fill="url(#shell)" stroke="{HAIR}"/><rect x="28" y="28" width="844" height="236" rx="18" fill="{PANEL}" stroke="#2f6fe0"/>
...
def signal_svg(cfg: dict, profile: dict, contribution: dict) -> str:
    ...
    for index, (label, value, color) in enumerate(metrics):
        ...
        cards.append(f'<g transform="translate({x} 83)"><rect width="190" height="112" rx="18" fill="{PANEL_2}" stroke="#2f6fe0"/><text x="22" y="30" class="label">{label}</text><text x="22" y="70" class="metric" fill="{color}"><tspan fill="{color}">{value}</tspan></text><rect x="22" y="88" width="144" height="7" rx="4" fill="#182e57"/><rect x="22" y="88" width="{progress}" height="7" rx="4" fill="{color}" class="bar" style="animation-delay:{index * .1:.2f}s"/></g>')
    ...
    for index, (language, count) in enumerate(languages[:5]):
        ...
        color = palette[index]
        language_nodes.append(f'<circle cx="62" cy="{y - 4}" r="5" fill="{color}"/><text x="82" y="{y}" class="language">{escape(language)}</text><text x="285" y="{y}" text-anchor="end" class="percent" fill="{color}"><tspan fill="{color}">{percent}%</tspan></text><rect x="315" y="{y - 12}" width="510" height="9" rx="5" fill="#1a3160"/><rect x="315" y="{y - 12}" width="{width}" height="9" rx="5" fill="{color}" class="bar" style="animation-delay:{.15 + index * .08:.2f}s"/>')
...
<defs><linearGradient id="shell" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#123a8f"/><stop offset=".55" stop-color="#0c1f4a"/><stop offset="1" stop-color="#2d5fbd"/></linearGradient><style>{css}</style></defs>
<rect width="900" height="520" rx="24" fill="{OUTER}"/><rect x="6" y="6" width="888" height="238" rx="20" fill="url(#shell)" stroke="{HAIR}"/><rect x="24" y="24" width="852" height="202" rx="18" fill="{PANEL}" stroke="#2f6fe0"/><text x="42" y="61" class="title">Profile Signal</text><text x="43" y="78" class="label">LIVE GITHUB STATS / @{escape(cfg['username'].upper())}</text>{''.join(cards)}
<rect x="6" y="258" width="888" height="256" rx="20" fill="url(#shell)" stroke="{HAIR}"/><rect x="24" y="276" width="852" height="220" rx="18" fill="{PANEL}" stroke="#2f6fe0"/><text x="43" y="312" class="title">Language Stack</text><text x="43" y="330" class="label">REPOSITORY-WEIGHTED TECHNOLOGIES</text><text x="849" y="312" text-anchor="end" class="label">&gt; STACK.SCAN</text>{''.join(language_nodes)}
